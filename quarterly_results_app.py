...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("v4.0 · JM Financial Internal Tool · Mistral AI")
...
2.  Other Income
3.  Total Income         -> (Calculate: Revenue from Operations + Other Income)
4.  Total Exp            -> (Total Expenses raw value from PDF)
...
10. Provisions           -> (Provisions & Contingencies)
11. Share of Profit of Associate/JV
12. Exceptional          -> (Exceptional Items)
...
def get_row(df: pd.DataFrame, label: str) -> pd.Series:
    """Helper to flexibly find rows even if the AI slightly misspells them."""
...
def build_template_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    pbt       = get_row(df, "PBT")
    tax       = get_row(df, "Tax")
    prov      = get_row(df, "Provisions")
    exc       = get_row(df, "Exceptional")
    pat       = get_row(df, "PAT")
    eps       = get_row(df, "EPS")
    
    # We also grab the RAW Total Expenses from the PDF to do the math ourselves
    raw_expenses = get_row(df, "Total Exp")
    ...
    all_cols = ["Q4FY2026","Q3FY2026","Q4FY2025","FY2026","FY2025"]
    ...
    for col in all_cols:
        ...
        raw_exp = float(raw_expenses.get(col, 0))

        # --- PYTHON CALCULATES THE MATH ---
        ti = ns + oi
        te = raw_exp - ir - dp 
        eb = ti - te
    ...
    def safe_pct(new_val, old_val):
        if old_val == 0 or pd.isna(new_val) or pd.isna(old_val) or new_val == "" or old_val == "": return 0
        return ((float(new_val) - float(old_val)) / abs(float(old_val))) * 100

    tdf["QoQ%"]     = tdf.apply(lambda r: safe_pct(r["Q4FY2026"], r["Q3FY2026"]) if r["Particulars"] != "" else "", axis=1)
    tdf["YoY%"]     = tdf.apply(lambda r: safe_pct(r["Q4FY2026"], r["Q4FY2025"]) if r["Particulars"] != "" else "", axis=1)
    tdf["% Change"] = tdf.apply(lambda r: safe_pct(r["FY2026"],   r["FY2025"])   if r["Particulars"] != "" else "", axis=1)
    
    return tdf[["Particulars","Q4FY2026","Q3FY2026","Q4FY2025","QoQ%","YoY%","FY2026","FY2025","% Change"]]
...
if extract_btn:
    if not api_key:
        ...
        with st.spinner("Parsing extracted data…"):
            ...
            # IMPORTANT: We build the template immediately to run the math
            tdf = build_template_df(df)
            st.session_state.df_extracted = tdf
            st.session_state.step = 2

        with st.spinner("Classifying result sentiment & updating log…"):
            try:
                pat_row = tdf[tdf["Particulars"] == "Reported Profit"].iloc[0]
                rev_row = tdf[tdf["Particulars"] == "Net Sales"].iloc[0]
                ...
                sentiment = "positive" if float(pat_row.get("YoY%", 0) if pat_row.get("YoY%", "") != "" else 0) >= 0 else "negative"
...
if st.session_state.df_extracted is not None:
    ...
    # KPI strip (using the structured template names)
    try:
        pat_row = edited_df[edited_df["Particulars"] == "Reported Profit"].iloc[0]
        eb_row  = edited_df[edited_df["Particulars"] == "EBIDTA"].iloc[0]
        rev_row = edited_df[edited_df["Particulars"] == "Net Sales"].iloc[0]
    ...
    with tab_word:
        ...
        if st.button("Build Word Document", key="build_word"):
            ...
            with st.spinner("Rendering table image…"):
                table_png = build_table_image(edited_df)
