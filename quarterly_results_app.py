...
def fetch_logo_direct(url: str) -> bytes | None:
    """
    raw.githubusercontent.com REJECTS requests that carry an
    Authorization header. Logos must be fetched WITHOUT auth headers.
    """
    try:
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("v4.2 · JM Financial Internal Tool · Mistral AI")
...
def call_mistral_vision(api_key: str, img: Image.Image) -> str:
    # Updated Prompt: Guides AI to extract intelligently rather than forcing rigid math formulas
...
Map the data from the image to these EXACT row names:
1.  Revenue from Operations
2.  Other Income
3.  Total Income
4.  Total Expenditure (Ex Int & Dep) -> (Calculate this as Total Income - EBITDA. If EBITDA is not given, find the subtotal of operating expenses before finance costs and depreciation).
5.  EBIDTA -> (Extract if given, otherwise calculate as Total Income - Total Expenditure Ex Int & Dep)
6.  Interest -> (Finance costs)
7.  Depreciation -> (Depreciation and amortisation)
8.  PBT -> (Profit before exceptional item and tax OR Profit before tax)
9.  Tax -> (Total tax expense)
10. Provisions & Contingencies
11. Exceptional Item
12. Reported Profit -> (Profit for the period / Net Profit)
13. EPS (Diluted)
...
def parse_mistral_csv(raw: str) -> pd.DataFrame:
    ...
    def safe_pct(new_val, old_val):
        ...
        return round(((new_val - old_val) / abs(old_val)) * 100)

    df["QoQ%"]     = df.apply(lambda r: safe_pct(r["Q4FY2026"], r["Q3FY2026"]), axis=1).astype(int)
    df["YoY%"]     = df.apply(lambda r: safe_pct(r["Q4FY2026"], r["Q4FY2025"]), axis=1).astype(int)
    df["% Change"] = df.apply(lambda r: safe_pct(r["FY2026"],   r["FY2025"]),   axis=1).astype(int)
    
    return df

def get_row(df: pd.DataFrame, label: str) -> pd.Series:
    row = df[df["Particulars"].str.strip().str.lower() == label.lower()]
    if not row.empty:
        return row.iloc[0]
    row = df[df["Particulars"].str.contains(label, case=False, na=False)]
...
def build_template_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    other_inc = get_row(df, "Other Income")
    tot_inc   = get_row(df, "Total Income")
    tot_exp   = get_row(df, "Total Expenditure (Ex Int & Dep)")
    ebitda    = get_row(df, "EBIDTA")
    interest  = get_row(df, "Interest")
    dep       = get_row(df, "Depreciation")
    pbt       = get_row(df, "PBT")
    tax       = get_row(df, "Tax")
    prov      = get_row(df, "Provisions & Contingencies")
    exc       = get_row(df, "Exceptional Item")
    pat       = get_row(df, "Reported Profit")
    eps       = get_row(df, "EPS")
    ...
    for col in all_cols:
        ns = float(net_sales.get(col, 0))
        oi = float(other_inc.get(col, 0))
        ti = float(tot_inc.get(col, 0))
        te = float(tot_exp.get(col, 0))
        eb = float(ebitda.get(col, 0))
        ir = float(interest.get(col, 0))
        dp = float(dep.get(col, 0))
        pb = float(pbt.get(col, 0))
        tx = float(tax.get(col, 0))
        pv = float(prov.get(col, 0))
        ex = float(exc.get(col, 0))
        pt = float(pat.get(col, 0))
        ep = float(eps.get(col, 0))
...
def generate_paragraphs(tdf: pd.DataFrame, company: str) -> list:
    def gv(row_label, col):
        r = tdf[tdf["Particulars"] == row_label]
        return float(r.iloc[0][col]) if not r.empty and r.iloc[0][col] != "" else 0

    ti_q426 = gv("Total Income", "Q4FY2026")
    ti_q326 = gv("Total Income", "Q3FY2026")
    ti_q425 = gv("Total Income", "Q4FY2025")
    ti_fy26 = gv("Total Income", "FY2026")
    ti_fy25 = gv("Total Income", "FY2025")
    ti_qoq  = gv("Total Income", "QoQ%")
    ti_yoy  = gv("Total Income", "YoY%")
    ti_chg  = gv("Total Income", "% Change")

    eb_q426 = gv("EBIDTA", "Q4FY2026")
    eb_q326 = gv("EBIDTA", "Q3FY2026")
    eb_q425 = gv("EBIDTA", "Q4FY2025")
    eb_fy26 = gv("EBIDTA", "FY2026")
    eb_fy25 = gv("EBIDTA", "FY2025")
    eb_qoq  = gv("EBIDTA", "QoQ%")
    eb_yoy  = gv("EBIDTA", "YoY%")
    eb_chg  = gv("EBIDTA", "% Change")

    pt_q426 = gv("Reported Profit", "Q4FY2026")
    pt_q326 = gv("Reported Profit", "Q3FY2026")
    pt_q425 = gv("Reported Profit", "Q4FY2025")
    pt_fy26 = gv("Reported Profit", "FY2026")
    pt_fy25 = gv("Reported Profit", "FY2025")
    pt_qoq  = gv("Reported Profit", "QoQ%")
    pt_yoy  = gv("Reported Profit", "YoY%")
    pt_chg  = gv("Reported Profit", "% Change")

    ep_q426 = gv("EPS (Diluted)", "Q4FY2026")
    ep_fy26 = gv("EPS (Diluted)", "FY2026")
    ep_qoq  = gv("EPS (Diluted)", "QoQ%")
    ...
    return [
        (f"{company} reported net profit of INR {fn(pt_q426)} Crs. in Q4FY2026, "
         f"{ud(pt_qoq)} by ~{fp(pt_qoq)}% QoQ & {ud(pt_yoy)} by ~{fp(pt_yoy)}% YoY "
         f"(Margin percentage {fn(pat_margin_q4)}%)."),
        (f"Company Reported Total Income of INR {fn(ti_q426)} Crs. in Q4FY2026 "
         f"as compared to INR {fn(ti_q326)} Crs. in previous quarter Q3FY2026. "
         f"Total Income has {udl(ti_qoq)} by {fp(ti_qoq)}% QoQ. "
         f"(Total Income of INR {fn(ti_q425)} Crs. in Q4FY2025 {udl(ti_yoy)} by {fp(ti_yoy)}% YoY) "
         f"For FY2026, Company has Reported Total Income of INR {fn(ti_fy26)} Crs. "
         f"as against INR {fn(ti_fy25)} Crs. in FY2025 {udl(ti_chg)} by ~{fp(ti_chg)}%."),
        (f"Company Reported EBITDA of INR {fn(eb_q426)} Crs. for Q4FY2026 "
         f"as compared to INR {fn(eb_q326)} in previous quarter Q3FY2026. "
         f"EBITDA has {udl(eb_qoq)} by {fp(eb_qoq)}% QoQ. "
         f"(EBIDTA of INR {fn(eb_q425)} Crs. in Q4FY2025 {udl(eb_yoy)} by {fp(eb_yoy)}% YoY) "
         f"EBIDTA margin stood at {fn(ebitda_margin_q4)}% for the quarter "
         f"{udl(ebitda_bps)} by {int(abs(ebitda_bps))} bps. "
         f"For FY2026, Company has Reported EBIDTA of INR {fn(eb_fy26)} Crs. "
         f"as against INR {fn(eb_fy25)} Crs. in FY2025 {ud(eb_chg)} by ~{fp(eb_chg)}%."),
        (f"Company Reported Profit of INR {fn(pt_q426)} Crs. for Q4FY2026 "
         f"as compared to INR {fn(pt_q326)} Crs. in previous quarter Q3FY2026. "
         f"Profit has {udl(pt_qoq)} by {fp(pt_qoq)}% QoQ. "
         f"(Profit of INR {fn(pt_q425)} Crs. in Q4FY2025 {udl(pt_yoy)} by {fp(pt_yoy)}% YoY.) "
         f"Profit margin stood at {fn(pat_margin_q4)}% for the quarter "
         f"{udl(pat_bps)} by {int(abs(pat_bps))} bps. "
         f"For FY2026, Company has Reported Profit of INR {fn(pt_fy26)} Crs. "
         f"as against INR {fn(pt_fy25)} Crs. in FY2025 {ud(pt_chg)} by ~{fp(pt_chg)}%."),
        (f"Company reported EPS of INR {fn(ep_q426)} "
         f"{udl(ep_qoq)} by {fp(ep_qoq)}% QoQ & "
         f"for FY2026 EPS stood at INR {fn(ep_fy26)}."),
    ]
...
def build_excel(tdf: pd.DataFrame, company: str) -> bytes:
...
if extract_btn:
    if not api_key:
        ...
        with st.spinner("Parsing extracted data…"):
            ...
            # Store raw parsed DF in session state to maintain editability in Step 2
            st.session_state.df_extracted = df
            st.session_state.step = 2

        with st.spinner("Classifying result sentiment & updating log…"):
            try:
                pat_row = df[df["Particulars"].str.contains("Reported Profit|PAT", case=False, na=False)].iloc[0]
                rev_row = df[df["Particulars"].str.contains("Revenue from Operations|Net Sales", case=False, na=False)].iloc[0]
                sentiment = call_mistral_sentiment(
                    api_key,
                    pat_yoy=float(pat_row.get("YoY%", 0) if pat_row.get("YoY%", "") != "" else 0),
                    pat_qoq=float(pat_row.get("QoQ%", 0) if pat_row.get("QoQ%", "") != "" else 0),
                    revenue_yoy=float(rev_row.get("YoY%", 0) if rev_row.get("YoY%", "") != "" else 0),
            ...
            try:
                new_entry = {
                    ...
                    "revenue":   f"₹{float(rev_row.get('Q4FY2026', 0) if rev_row.get('Q4FY2026', '') != '' else 0):,.0f} Cr",
                    "pat":       f"₹{float(pat_row.get('Q4FY2026', 0) if pat_row.get('Q4FY2026', '') != '' else 0):,.0f} Cr",
                    "sentiment": sentiment,
                    "date":      datetime.date.today().strftime("%d %b %Y"),
                    "pat_yoy":   f"{float(pat_row.get('YoY%', 0) if pat_row.get('YoY%', '') != '' else 0):+.0f}%",
                    "rev_yoy":   f"{float(rev_row.get('YoY%', 0) if rev_row.get('YoY%', '') != '' else 0):+.0f}%",
                }
                ...
                fetch_results_log.clear()
                 ...
                 st.markdown(f'<div class="status-warn">⚠️ Log update failed: {e}</div>', unsafe_allow_html=True)
...
if st.session_state.df_extracted is not None:
    ...
    # Present the raw dataframe to the user for editing to maintain full Streamlit UI compatibility
    ...
    st.session_state.df_extracted = edited_df

    # Convert the edited raw data into the final structured template for docs & KPIs
    final_template_df = build_template_df(edited_df)

    # KPI strip
    try:
        pat_row = get_row(final_template_df, "Reported Profit")
        eb_row  = get_row(final_template_df, "EBIDTA")
        rev_row = get_row(final_template_df, "Net Sales")
        
        rev_val = float(rev_row.get("Q4FY2026", 0) if rev_row.get("Q4FY2026", "") != "" else 0)
        eb_val  = float(eb_row.get("Q4FY2026", 0) if eb_row.get("Q4FY2026", "") != "" else 0)
        pat_val = float(pat_row.get("Q4FY2026", 0) if pat_row.get("Q4FY2026", "") != "" else 0)
        
        # Pull growth percentages directly from the safely calculated columns
        pat_qoq = float(pat_row.get("QoQ%", 0) if pat_row.get("QoQ%", "") != "" else 0)
        pat_yoy = float(pat_row.get("YoY%", 0) if pat_row.get("YoY%", "") != "" else 0)

        eb_mg   = (eb_val / rev_val * 100) if rev_val else 0
        pt_mg   = (pat_val / rev_val * 100) if rev_val else 0
        
        st.markdown(f"""
        <div class="metric-row">
          <div class="metric-card"><div class="label">Revenue (Q4FY26)</div>
            <div class="value">₹{rev_val:,.0f}</div><div class="sub">Crs.</div></div>
          <div class="metric-card"><div class="label">EBITDA (Q4FY26)</div>
            <div class="value">₹{eb_val:,.0f}</div><div class="sub">{eb_mg:.1f}% margin</div></div>
          <div class="metric-card"><div class="label">PAT (Q4FY26)</div>
            <div class="value">₹{pat_val:,.0f}</div><div class="sub">{pt_mg:.1f}% margin</div></div>
          <div class="metric-card"><div class="label">PAT QoQ</div>
            <div class="value">{pat_qoq:+.0f}%</div><div class="sub">vs Q3FY26</div></div>
          <div class="metric-card"><div class="label">PAT YoY</div>
            <div class="value">{pat_yoy:+.0f}%</div><div class="sub">vs Q4FY25</div></div>
    ...
    with tab_excel:
        ...
        if st.button("Build Excel", key="build_excel"):
            with st.spinner("Building Excel…"):
                xl_bytes = build_excel(final_template_df, company_name)
            st.download_button(
    ...
    with tab_word:
        ...
        paras = generate_paragraphs(final_template_df, company_name)
        ...
        if st.button("Build Word Document", key="build_word"):
            ...
            with st.spinner("Rendering table image…"):
                table_png = build_table_image(final_template_df)
