...
with st.sidebar:
    ...
    st.caption("Left: Q4.png · Right: JM Logo.png")
...
MISTRAL_VISION_MODEL = "pixtral-12b-2409"   # Mistral's vision model
MISTRAL_TEXT_MODEL   = "mistral-large-latest"  # For text summarisation
...
def extract_pdf_full_text(pdf_bytes: bytes) -> str:
    """Extract all text from the PDF for AI summarisation."""
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    text_parts = []
    for page in doc:
...
def call_mistral_vision(api_key: str, img: Image.Image) -> str:
    """Call Mistral Pixtral vision model to extract financial table from image."""
...
def call_mistral_ai_summary(api_key: str, pdf_text: str, company: str) -> str:
    """
    Call Mistral text model with the risk analyst prompt to generate
    a bullet-point summary of key matters + 2-liner financial view.
    """
    system_prompt = (
    ...
    user_prompt = (
        ...
        "Please provide:\n"
        "1. Key matters (acquisitions, penalties, litigations, one-time items, management commentary highlights) "
        "in bullet points\n"
...
def add_paragraph_with_font(doc, text: str, bold: bool = False,
                             font_name: str = "Segoe UI", font_size_pt: int = 12,
                             align=WD_ALIGN_PARAGRAPH.JUSTIFY):
    """Helper: add a paragraph with consistent Segoe UI 12pt styling."""
    p   = doc.add_paragraph()
    p.alignment = align
    run = p.add_run(text)
...
def build_word_doc(
    ...
    # ── AI Risk Analyst Summary (below table image) ──────────────────
    if ai_summary and ai_summary.strip():
        ...
        h_run = heading_p.add_run("Risk Analyst Commentary (AI-Generated)")
        h_run.font.name = "Segoe UI"
        h_run.font.size = Pt(12)
        h_run.bold      = True
        h_run.font.color.rgb = RGBColor(0x1A, 0x1A, 0x2E)

        # Summary content — each line as its own paragraph
...
def build_excel(df: pd.DataFrame, company: str) -> bytes:
    """Styled Excel output using openpyxl."""
    ...
    if not api_key:
        ...
        with st.spinner("Generating AI Risk Analyst Summary…"):
            try:
                ...
                st.warning(f"AI summary generation failed (will continue without it): {e}")
                st.session_state.ai_summary = ""

        with st.spinner("Parsing Mistral response…"):
...
if st.session_state.df_extracted is not None:
    ...
    # Display AI Summary in Streamlit UI
    if st.session_state.ai_summary:
        st.markdown("---")
        st.markdown("#### 🤖 AI Risk Analyst Summary")
        st.markdown(
            f'<div class="ai-summary-box"><h4>Key Matters & Analyst View</h4>{st.session_state.ai_summary.replace(chr(10), "<br>")}</div>',
            unsafe_allow_html=True
        )
        st.caption("This summary will be included below the table in the Word document.")
    ...
    with tab_word:
        st.markdown(
            ...
            "coloured table, and AI Risk Analyst Summary below the table."
        )
        ...
        # Show AI summary for review/edit
        if st.session_state.ai_summary:
            st.markdown("**AI Risk Analyst Summary** *(editable before generating Word)*")
            ai_summary_edited = st.text_area(
                "AI Summary (will appear below table in Word doc)",
        ...
        st.caption("Edit any content above before generating the Word document. Font: Segoe UI 12pt.")

        if st.button("Build Word Document", key="build_word"):
            # Collect possibly-edited content
            ...
            with st.spinner("Building Word document…"):
                docx_bytes = build_word_doc(
                    ...
                    left_logo_bytes=left_logo_bytes,
                    right_logo_bytes=right_logo_bytes
                )
